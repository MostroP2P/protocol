...
Clients can request detailed information for existing orders by sending a message with the action `orders` (wrapped in the [active transport](./overview.md#transports)). This is useful for refreshing stale UI state or restoring a session from the mnemonic seed on a new device.
